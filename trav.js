(function(global, factory) {
    ...
    var utils = (function() {
        ...
        var getInfo = function(output, ff) {
            var d = [];
            $.each(ff, function(idx, obj) {
                d[idx] = $.Deferred();
            });
            ...
            $.when.apply($, d).done(function() {
                ...
                json.features = ff;
                saveFile(output, JSON.stringify(json));
            });
            ...
            $.each(ff, function(idx, obj) {
                ...
                setTimeout(function() {
                    console.log(idx);
                    map.findLatLng(obj.properties.name, gotLocaltion);
                }, tm);

                function gotLocaltion(location) {
                    if (location) {
                        obj["geometry"] = {
                            "type": "Point",
                        ...
                        map.findAddress(location, gotAddr);
                    } else {
                        d[idx].resolve();
                    }
                }

                function gotAddr(address) {
                    if (address) {
                        console.log(address);
        ...
        var convert = function(output, text) {
            var ff = [];
            var n = -1;
...
$(document).ready(function() {
    ...
    document.getElementById('file').onchange = function() {
        ...
        trav.utils.process(file, "itry.json");
    };
});
